...
- Critical stages (fail the CI run with a non-zero exit): fetch (all sources down), filter, summarize, render_check, publish (English), translate_fa (Farsi). Editorial degrades to the unedited draft and records a `degraded` flag in run_meta instead of failing silently.
- Render check (`stages/render_check.py`): runs after verify, before publish. Kramdown (with `parse_block_html: true`) mis-renders bare `<details>`/`<summary>` tags (the brief collapses into the summary and literal `</summary>` text shows) and bare `|` separators (the Sources line becomes a table). The stage auto-repairs missing `markdown="block"`/`markdown="span"` attributes and escapes source-line pipes, then fails the run if structural damage remains. On failure, publish/translate_fa/mailer are skipped so no broken page or email ships. `translate_fa` runs the same checks on the Farsi body and additionally rejects reasoning-dump output (marker/script/length heuristics).
- Backfilled posts: `run_meta.json` records `backfill: true` for backfill runs. Publish adds `backfilled: true` frontmatter plus a "(backfilled)" title suffix; translate_fa adds the Farsi "(با تأخیر)" suffix. The post layout, index pages, and prev/next nav append the marker to displayed dates.
